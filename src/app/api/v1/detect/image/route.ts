...
export async function POST(request: Request): Promise<Response> {
  ...
  const rl = rateLimitCheck(auth.tenantId, 'detect.image');
  if (!rl.allowed) {
  ...
  try {
    ...
    // Try to extract an existing watermark
    const extraction = await extractImageWatermark(buffer);
    ...
    return Response.json({
      ...
      confidence: watermarkFound ? 1.0 : null,
      provider: extraction?.provider ?? null,
      model: extraction?.model ?? null,
    }, { status: 200 });
  } catch (err) {
    console.error('[detect/image] Error:', err);
    return Response.json({ error: 'Internal server error during image detection.' }, { status: 500 });
  }
}
